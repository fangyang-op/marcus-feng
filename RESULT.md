...
## 第四輪強化 — 補頁、對齊真實 UI、上鎖邏輯(2026-06-17)

依使用者貼的兩張真實 Nexus 畫面與補頁清單。`npm run build` 成功(7 路由全靜態);逐一點過每個新 nav 與互動皆驗證通過;安全掃描再次乾淨。

### CRM
- **進階分頁上鎖邏輯**(符合 CRM 流程):學生狀態為招生階段(new_lead/contacted/consulting/qualified,或 paused 未成交)時,「成交 / 選校表 / 文件 / 申請」四分頁顯示 🔒、disabled、點了不切換,並顯示「成交後自動解鎖」說明橫幅;狀態為 closed / application 時解鎖。概覽 / 時間軸 / 成績永遠可用。**實測**:諮詢中學生四分頁鎖定、送件中學生解鎖。
- **新增「內部封測 (UAT)」頁**(nav 帶 badge 4):4 張統計卡(總 24 / 通過 17 / 異常 3 / 未測 4)+ 6 章節 24 測試項(狀態 Pill 通過·異常·未測 + 回報鈕)。
- **設定「轉介人 / 名單來源」做成真子頁**:轉介人 10 筆、名單來源 8 筆(列表 + 搜尋 + 返回設定),其餘 5 張設定卡維持 DemoModal。

### Nexus(+4 頁,nav 11 項)
- **新增頁**:最新公告(12 則,點開 Modal)、顧問知識庫(16 篇,分類+搜尋+Modal)、AI 設定(API Key 遮罩/模型選擇/向量化工具)、權限管理系統(18 使用者,狀態可切換 toast)。
- **歷屆榜單查詢**對齊真實 UI:即時同步副標、「已向量化 593」徽章、匯出 / 同步 / 垃圾桶 / 齒輪(no-op→toast/modal)、總覽/2026/2025/2024 tabs、可收合「展開搜尋與國家過濾」、累積獎學金 NTD$32,427,333.74、大碩/洋碩/獎學金 checkbox、國家快篩 全部/AUS/CAD/Ireland/SCT/UK/USA;**34 筆**脫敏假資料。實測篩選即時生效(全部 34→USA 9→USA+洋碩 1)。
- **常用院校資料庫**對齊真實 UI:「尚有 5 筆未維護」、向量化(294/302)徽章、匯出 / 新增校所、可收合「展開搜尋與成績篩選」、GPA/TOEFL/IELTS Min 輸入、排序方式(成功錄取數/US News/QS/GPA/TOEFL/IELTS);**24 所**含成功錄取數。實測 GPA Min 即時過濾(24→23)。

### Matrix(+1 頁,nav 7 項)
- **新增「組織人效統計」頁**:4 大區 × 20 顧問;全域 toggle 營收佔比(每區 PieChart)↔ 月度趨勢(每區 LineChart:區總營收虛線 + 各顧問彩線)+ 同期比較疊三年度;4 張 StatCard。**修掉一個真 bug**:原 Fragment 包多條 `<Line>` 導致折線畫不出,改攤平成陣列後正常渲染。
- **人員編制設定加厚**:每區 5 位顧問(共 20),含到職日、在職率小總表;狀態切換 toast、新增編制 modal。

### 驗證 & 安全
逐一點過三個 Demo 全部 nav(CRM 6 / Nexus 11 / Matrix 7)皆渲染內容、無 404、無 console 錯誤;新按鈕皆有回應。安全掃描:demo seed data 無真實員工/客戶/金鑰/端點洩漏,email 假網域,姓名為假名 / 全形遮罩。
